...
def getCRLB(theta,gain,rmu,rvar,texp,eta,L=19,nchains=20,niter=1000,skip=5,tburn=500,take=500,plot=True):

    x0,y0,sigma,N0,B0 = theta
    I0 = eta*N0*texp
    B = eta*B0*texp
    psf = GaussianPSF(x0,y0,L,I0,B,sigma,gain,rmu,rvar)
    adu = psf.generate(plot=plot)
    ...
    for n in range(take):
        fisher[n] = hessian_autograd(samples[:,n], counts, eta, texp)
